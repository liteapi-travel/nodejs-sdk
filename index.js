class LiteApi {
   ...
   * @param {string[]} data.hotelIds - List of hotel IDs to retrieve rates for.
  ...
  async getFullRates(data, timeoutMs = 10000) {
    ...
    if (!data || typeof data !== "object") {
      ...
      if (!data.checkin || typeof data.checkin !== "string") {
        errors.push(
          "`checkin` is required and must be a string in YYYY-MM-DD format."
        );
      }
      if (!data.checkout || typeof data.checkout !== "string") {
        errors.push(
          "`checkout` is required and must be a string in YYYY-MM-DD format."
        );
      }
      if (!data.currency || typeof data.currency !== "string") {
        errors.push(
          '`currency` is required and must be a string (e.g., "USD").'
        );
      }
      if (!data.guestNationality || typeof data.guestNationality !== "string") {
        errors.push(
          '`guestNationality` is required and must be a string (e.g., "US").'
        );
      }
        ...
        data.occupancies.forEach((occupancy, index) => {
          if (typeof occupancy !== "object") {
            ...
            if (typeof occupancy.adults !== "number") {
              errors.push(
                `occupancies[${index}] must have a numeric 'adults' property.`
              );
            }
            // If children are used, ensure it is an array of numbers
            if (occupancy.children && !Array.isArray(occupancy.children)) {
              errors.push(
                `occupancies[${index}].children must be an array of numbers if provided.`
              );
            }
          }
        });
    ...
    try {
      ...
      // ** FIX: Parse the response as text, then safely JSON-parse it **
      const text = await response.text();
      let result;
      try {
        result = JSON.parse(text);
      } catch (parseError) {
        // If it might be a rate-limit or invalid JSON, handle it gracefully
        if (response.status === 429 || text.includes("Limit exceeded")) {
          // Return a clean JSON structure for rate-limit scenario
          return {
            status: "failed",
  ...
  async book(data, timeoutMs = 10000) {
    ...
    if (!data || typeof data !== "object") {
      ...
      if (!data.payment || typeof data.payment !== "object") {
        ...
        !data.payment.method ||
        typeof data.payment.method !== "string"
      ) {
  ...
  async getBookingsList(clientReference) {
    ...
    const response = await fetch(
      this.bookServiceURL + "/bookings?clientReference=" + clientReference,
      options
    );
  ...
  async retrieveBooking(bookingId) {
    ...
    const response = await fetch(
      this.bookServiceURL + "/bookings/" + bookingId,
      options
    );
    const data = await response.json();
    if (!response.ok) {
  ...
  async getCitiesByCountryCode(countryCode) {
    ...
    const response = await fetch(
      this.serviceURL + "/data/cities?countryCode=" + countryCode,
      options
    );
    const data = await response.json();
    if (!response.ok) {
  ...
  async getHotels(parameters) {
    ...
    const response = await fetch(
      this.serviceURL + "/data/hotels?" + query,
      options
    );
    const data = await response.json();
  ...
  async getHotelDetails(hotelId) {
    ...
    const response = await fetch(
      this.serviceURL + "/data/hotel?hotelId=" + hotelId,
      options
    );
    const data = await response.json();
    if (!response.ok) {
  ...
  async getDataReviews(hotelId, limit, getSentiment) {
    ...
    try {
      const response = await fetch(
        `${this.serviceURL}/data/reviews?hotelId=${hotelId}&limit=${limit}&getSentiment=${getSentiment}`,
        options
      );
  ...
  async getGuestsIds(guestId) {
    ...
    const response = await fetch(
      this.serviceURL + "/guests/" + guestId,
      options
    );
    const data = await response.json();
    if (!response.ok) {
  ...
  async getGuestsBookings(guestId) {
    ...
    const response = await fetch(
      this.serviceURL + "/guests/" + guestId + "/bookings",
      options
    );
    const data = await response.json();
    if (!response.ok) {
  ...
  async getVoucherById(voucherID) {
    ...
    const response = await fetch(
      this.dashboardURL + "/vouchers/" + voucherID,
      options
    );
    const result = await response.json();
  ...
  async updateVoucherStatus(id, data) {
    ...
    const response = await fetch(
      this.dashboardURL + "/vouchers/" + id + "/status",
      options
    );
    const result = await response.json();
    ...
    if (!response.ok) {
      ...
      return {
        ...
        error:
          result.error ||
          "An error occurred while updating the voucher status.",
      };
    }
  ...
  async retrieveWeeklyAnalytics(data) {
    ...
    const response = await fetch(
      `${this.dashboardURL}/analytics/weekly`,
      options
    );
    const result = await response.json();
  ...
  async retrieveAnalyticsReport(data) {
    ...
    const response = await fetch(
      `${this.dashboardURL}/analytics/report`,
      options
    );
    const result = await response.json();
  ...
  async retrieveMarketAnalytics(data) {
    ...
    const response = await fetch(
      `${this.dashboardURL}/analytics/markets`,
      options
    );
    const result = await response.json();
  ...
  async retrieveMostBookedHotels(data) {
    ...
    const response = await fetch(
      `${this.dashboardURL}/analytics/hotels`,
      options
    );
    const result = await response.json();
